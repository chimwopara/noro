...
(function initReviewStack() {
  ...
  if (!grid) return;
  const cards = Array.from(grid.querySelectorAll('.r-card'));
  const n = cards.length;
  if (n < 2) return;

  // Strip reveal so cards appear immediately
  cards.forEach(c => { c.classList.remove('reveal'); c.classList.add('visible'); });

  let current = 0;

  // Wrap grid for touch isolation
  const wrap = document.createElement('div');
  wrap.className = 'reviews-stack-wrap';
  grid.parentNode.insertBefore(wrap, grid);
  wrap.appendChild(grid);

  // Dot indicator
  const dotsWrap = document.createElement('div');
  dotsWrap.className = 'rev-stack-dots';
  const dotEls = cards.map((_, i) => {
    const d = document.createElement('div');
    d.className = 'rev-stack-dot' + (i === 0 ? ' active' : '');
  ...
  function setGridHeight() {
    // Height = front card + stack peek offsets
    const h = cards[current].offsetHeight;
    const peek = Math.min(2, n - 1 - current) * 8;
    grid.style.height = (h + peek) + 'px';
  }
  ...
  function render() {
    cards.forEach((card, i) => {
      ...
      if (offset < 0) {
        card.style.transform = 'translateX(-108%) scale(0.88)';
        ...
        card.style.transform = 'translateY(14px) scale(0.92)';
  ...
  grid.addEventListener('touchend', e => {
    ...
    if (dy > Math.abs(dx) * 1.4) return; // vertical scroll, ignore
    if (dx < -44 && current < n - 1) goTo(current + 1);
    else if (dx > 44 && current > 0) goTo(current - 1);
  }, { passive: true });

  render();
  window.addEventListener('resize', () => { if (window.innerWidth <= 600) setGridHeight(); }, { passive: true });
})();
